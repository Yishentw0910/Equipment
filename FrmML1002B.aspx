<%@ Page Language="C#" AutoEventWireup="true" CodeFile="FrmML1002B.aspx.cs" Inherits="FrmML1002B" %>
...
<body onkeydown="body_OnKeyDown(event)">
  ...
  <div>
    ...
    <div class="div_Info4">
      <table width="100%" cellpadding="1" cellspacing="1" class="tab_query">
        <tr>
          ...
          <td width="30%">
            <asp:TextBox ID="txtCustomerID" onkeypress="OnlyNumDUCase(this);" runat="server" CssClass="txt_normal" Width="80%"></asp:TextBox>
          </td>
          <th width="20%">
            客戶名稱：
          </th>
        ...
        <asp:Repeater ID="rptCase" runat="server">
          <ItemTemplate>
            <tr onmouseover="MouseOver(event)" onmouseout="MouseOut(event)" onclick="MouseDown(event);Row_Click(this)"
              ...
              <td width="27%"><%# Eval("CUSTNAME")%>
                <!-- CssClass="txt_table_readonly" -->
                <asp:TextBox CssClass="txt_QueryListForName" style="display:none;" ReadOnly="true" ID="lblCONTACTNAME"
                  Width="160px" runat="server" Text='<%# Eval("CUSTNAME")%>'></asp:TextBox>
              </td>
              <td>
                <%#Eval("CASEID")%>
              </td>
    ...
    <div class="btn_div">
      <asp:Button ID="Button5" runat="server" Text="載入案件" CssClass="btn_normal" OnClientClick="return cmdConfirm_Click();" />
    </div>
  </div>
  </form>
</body>
